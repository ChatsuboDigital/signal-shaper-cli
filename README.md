...
**A signal-driven matching engine for B2B outreach.**

Companies constantly broadcast what they need — job posts, funding announcements, new hires, product launches. Signalis reads those signals, finds who's best positioned to help, and makes the introduction at exactly the right moment.

```
  ⚗  Shaper      Raw data → normalised, signal-enriched CSVs
  ⚯  Connector   Supply + Demand → matched pairs, AI intros, campaigns
```

**The mechanic:** Shape your supply list (the people offering services) and demand list (the companies that need them) with real-time signals. The Connector scores every possible pair against those signals, finds missing emails, writes a specific intro referencing the actual trigger, and sends it. The signal is the reason for the outreach — when the match is right, the copy writes itself.
...
```bash
git clone https://github.com/ChatsuboDigital/signalis.git
cd signalis
...
```
  1  ⚗  Shaper       Shape raw data into signal-enriched CSVs
  2  ⚯  Connector    Match supply & demand · write intros · send campaigns
...
| `signalis connector run --demand demand.csv --supply supply.csv` | Run the connector directly |

---

## ⚗  Shaper

Takes raw contact data and outputs clean, signal-enriched CSVs ready for the Connector. Auto-detects columns, cleans domains and names, resolves missing data, and generates why-now signals per company using Exa AI.
...
```
  Raw input (CSV or Apify dataset)
      ...
      ├─ 5. Signal & ctx   Set signals manually, map from a column, or generate via Exa
...
This is the universal format. The Connector expects it on both sides.

---

## ⚯  Connector

Where supply meets demand. Takes two shaped CSVs — one for the people offering services (supply), one for the companies that need them (demand) — and runs a pipeline that scores every possible pair, finds missing emails, generates AI intros, and pushes leads to your campaign platform.

```
  supply.csv  +  demand.csv
      │
      ├─ 1. Normalize        Standardise both datasets
      ├─ 2. Match            Score every supply–demand pair against signals
...
Missing emails are resolved through a cascading system — cache first, then Apollo, then Anymail Finder, then SSM as final fallback. Successful results are cached locally for 90 days, so re-runs on the same contacts cost zero API calls.
...
```bash
signalis connector cache          # View cache stats
signalis connector cache-clear    # Wipe the cache
```

### AI intros

For each match, an AI model (OpenAI or Anthropic — your choice) generates personalised email intros using the match context. Two intros per pair: one for each side. The copy references the actual signal — not a template, a specific observation about what's happening at that company right now.
...
A **signal** is a short statement describing why someone is worth reaching out to *right now*. It's the timing trigger that makes outreach relevant instead of random.
...
**`signalis connector` not found** — Run `pip install -e ".[all]"` from the project folder (with venv active) to register the connector. The `update` command does this automatically.
