"""
Signalis — Signal-Driven Matching Engine for B2B Outreach

Shape your supply and demand data with real-time signals, then connect
the right pairs at exactly the right moment.
"""
...
def cli(ctx):
    ...
    Shape your supply and demand data with real-time signals, then let the
    Connector score every pair, find missing emails, write the intros, and
    push leads directly to your campaign platform.

    Run without arguments for the interactive menu, or use:
      signalis interactive  - Interactive guided workflow
      signalis connector    - Match supply & demand · write intros · send campaigns
...
if _has_connector:
    cli.add_command(connector_cli, name='connector')
    ...
    try:
        while True:  # top-level restart loop
            ...
            while True:
                ...
                console.print("  [cyan]1[/cyan]  [bold]⚗  Shaper[/bold]      [dim]Shape raw data into signal-enriched CSVs[/dim]")
                console.print("  [cyan]2[/cyan]  [bold]⚯  Connector[/bold]   [dim]Match supply & demand · write intros · send campaigns[/dim]")
                ...
                if top_choice == "0":
                    console.print("\n[yellow]Exiting Signalis...[/yellow]\n")
                    return
            ...
            for idx, data_type in enumerate(datasets_to_process, 1):
                ...
                if confidence >= 0.8 and auto_mapping.is_complete():
                    ...
                    for target, source in mapping_summary.items():
                        if target in {'signal', 'company_description'}:
                            continue  # always Step 5
...
def _process_records(records, mapping, global_signal=None, signal_prefix=None):
    ...
    with create_progress() as progress:
        ...
        for record in records:
            ...
            if full_name_field:
                raw_full_name = record.get(full_name_field, '')
                if isinstance(raw_full_name, dict):
                    raw_full_name = raw_full_name.get('name') or raw_full_name.get('fullName') or ''
                elif isinstance(raw_full_name, str) and raw_full_name.strip().startswith('{'):
                    try:
                        parsed = ast.literal_eval(raw_full_name)
                        if isinstance(parsed, dict):
                            raw_full_name = parsed.get('name') or parsed.get('fullName') or ''
                    except Exception:
                        pass
                full_name = normalize_field(raw_full_name, 'name')
            ...
            if company_field:
                raw_company = record.get(company_field, '')
                # Apify scrapers (e.g. Wellfound) sometimes return company as a
                # nested dict — extract the name field rather than str(dict).
                # When loaded from CSV the dict is stringified; handle both forms.
                if isinstance(raw_company, dict):
                    raw_company = raw_company.get('name') or raw_company.get('title') or ''
                elif isinstance(raw_company, str) and raw_company.strip().startswith('{'):
                    try:
                        parsed = ast.literal_eval(raw_company)
                        if isinstance(parsed, dict):
                            raw_company = parsed.get('name') or parsed.get('title') or ''
                    except Exception:
                        pass
                processed['company'] = normalize_field(raw_company, 'text')
...
def version():
    ...
    click.echo(f"Signalis v{__version__}")
...
def config():
    ...
    console.print("\n[bold cyan]Signalis Configuration[/bold cyan]\n")
...
def update():
    ...
    if not (root / '.git').exists():
        ...
        show_info("Re-install from: https://github.com/ChatsuboDigital/signalis.git")
        return
